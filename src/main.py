...
from collector import collect_all, PROTOCOLS
...
def get_host_port_sni(cfg: str) -> tuple[str, int, str | None] | None:
    """Извлечь (host, port, sni) из конфига для тестирования."""
    try:
...
async def main():
    ...
    log.info("=" * 62)
    log.info("🚀 VLESS Collector — старт пайплайна")
    log.info("=" * 62)
    ...
    cfg_by_target: dict[tuple[str, int], list[str]] = {}
    for cfg in unique:
        t = get_host_port_sni(cfg)
        if t:
            targets.append(t)
            key = (t[0], t[1])
            cfg_by_target.setdefault(key, []).append(cfg)
    ...
    # Собираем рабочие конфиги с задержками
    ...
    for r in test_results:
        ...
        # Добавляем все конфиги для этого (host, port)
        for cfg in cfg_by_target.get((host, port), []):
            working.append((cfg, latency))

    # Дедуплицируем финальный список и сортируем по задержке
    seen_final: set[str] = set()
    working_dedup: list[tuple[str, int]] = []
    for cfg, lat in sorted(working, key=lambda x: x[1]):
        key = cfg.split("#")[0].rstrip("?& ")
        if key not in seen_final:
            seen_final.add(key)
            working_dedup.append((cfg, lat))

    log.info("✅ Рабочих серверов: %d", len(working_dedup))
    ...
    # ── 5. Геолокация ─────────────────────────────────────────────────────────
    hosts = []
    for cfg, _ in working_dedup:
        try:
            h = urlparse(cfg).hostname or ""
            if h:
                hosts.append(h)
        except Exception:
            pass

    geo_map = await geolocate_hosts(list(set(hosts)))

    # ── 6. Запись файлов ──────────────────────────────────────────────────────
    stats = write_all_outputs(working_dedup, geo_map=geo_map, tls_map=tls_map)
    ...
    log.info("=" * 62)
    log.info("🏁 Готово за %d сек.  Рабочих: %d  TLS-ок: %d",
